"""E0_ema21H4 (vtrend_ema21) — E0 + EMA(21d) regime filter on H4 bars.

Same as E0 with an additional entry condition:
  close > EMA(ema_regime_period) on H4 bars.

EMA(21d) = EMA(126) on H4 data (21 days * 6 bars/day).
...
Parameters (tunable):
  ...
  ema_regime_period -- EMA regime filter period in H4 bars (default: 126 = 21d)
...
class VTrendEma21Strategy(Strategy):
    def __init__(self, config: VTrendEma21Config | None = None) -> None:
        self._config = config or VTrendEma21Config()
        self._c = self._config

        # Precomputed indicator arrays
        self._ema_fast: np.ndarray | None = None
        self._ema_slow: np.ndarray | None = None
        self._ema_regime: np.ndarray | None = None
    ...
    def on_init(self, h4_bars: list, d1_bars: list) -> None:
        ...
        self._ema_regime = _ema(close, self._c.ema_regime_period)
    ...
    def on_bar(self, state: MarketState) -> Signal | None:
        ...
        if (self._ema_fast is None or self._atr is None or
                self._vdo is None or self._ema_regime is None or i < 1):
            return None

        ema_f = self._ema_fast[i]
        ema_s = self._ema_slow[i]
        ema_r = self._ema_regime[i]
        atr_val = self._atr[i]
        vdo_val = self._vdo[i]
        price = state.bar.close

        if math.isnan(atr_val) or math.isnan(ema_f) or math.isnan(ema_s) or math.isnan(ema_r):
            return None
        ...
        if not self._in_position:
            # ENTRY: trend up AND VDO confirms AND regime filter
            regime_ok = price > ema_r
            if trend_up and vdo_val > self._c.vdo_threshold and regime_ok:
                self._in_position = True
                self._peak_price = price
                return Signal(target_exposure=1.0, reason="vtrend_ema21_entry")
        else:
            self._peak_price = max(self._peak_price, price)

            # EXIT: trailing stop OR trend reversal (same as E0)
            trail_stop = self._peak_price - self._c.trail_mult * atr_val
            if price < trail_stop:
                self._in_position = False
                self._peak_price = 0.0
                return Signal(target_exposure=0.0, reason="vtrend_ema21_trail_stop")

            if trend_down:
                self._in_position = False
                self._peak_price = 0.0
                return Signal(target_exposure=0.0, reason="vtrend_ema21_trend_exit")
