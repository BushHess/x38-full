"""VTREND-X0-E5EXIT -- E0_ema21D1 entry + E5 robust ATR trail exit.

Near-identical to E5_ema21D1 (vtrend_e5_ema21_d1). Same entry as E0_ema21D1
(EMA crossover + VDO + D1 regime), but exit trail uses robust ATR (E5-style)
instead of standard ATR(14). Research origin: X0 series.
...
Exit (when long, E5-style robust ATR trail):
  1. Trail stop: close < peak_price - trail_mult * robust_ATR
  2. Trend reversal: ema_fast(H4) < ema_slow(H4)

Robust ATR: cap TR at rolling Q(ratr_cap_q) of prior ratr_cap_lb bars,
then compute Wilder EMA(ratr_period) on capped TR.
...
@dataclass
class VTrendX0E5ExitConfig:
    # Tunable (4 parameters, same as E0_ema21D1 / vtrend_x0)
...
class VTrendX0E5ExitStrategy(Strategy):
    def __init__(self, config: VTrendX0E5ExitConfig | None = None) -> None:
        self._config = config or VTrendX0E5ExitConfig()
