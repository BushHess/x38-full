"""E5_ema21D1_VC — E5 (robust ATR trail) + D1 EMA(21) regime + vol compression gate.
...
Combines:
  ...
  - Vol compression gate: entry only when vol_ratio_5_20 < compression_threshold
...
Parameters (tunable):
  ...
  compression_threshold -- max vol_ratio_5_20 for entry (default: 0.6)
...
Vol compression parameters (structural, not tunable):
  compression_fast -- 5 (rolling std window, short)
  compression_slow -- 20 (rolling std window, long)
"""
...
class VTrendE5Ema21D1VCConfig:
    # Tunable (5 parameters: 4 inherited + 1 new)
    ...
    # Vol compression structural (not tunable — fixed 5/20 windows)
    compression_fast: int = 5
    compression_slow: int = 20

    # Regime monitor (optional operational guard)
    enable_regime_monitor: bool = False


class VTrendE5Ema21D1VCStrategy(Strategy):
    def __init__(
        self,
        config: VTrendE5Ema21D1VCConfig | None = None,
        monitor_alerts: np.ndarray | None = None,
    ) -> None:
        self._config = config or VTrendE5Ema21D1VCConfig()
        ...
        # Vol compression ratio array
        self._vol_ratio: np.ndarray | None = None
    ...
    def on_bar(self, state: MarketState) -> Signal | None:
        ...
        if (self._ema_fast is None or self._ratr is None or
                ...
                self._vol_ratio is None or i < 1):
            return None
        ...
        if not self._in_position:
            # ENTRY: trend up AND VDO confirms AND D1 regime AND compression AND not RED
            ...
            if monitor_red:
                ...
                return Signal(target_exposure=0.0, reason="vtrend_e5_ema21_d1_vc_monitor_exit")
            ...
            if price < trail_stop:
                self._in_position = False
                self._peak_price = 0.0
                return Signal(target_exposure=0.0, reason="vtrend_e5_ema21_d1_vc_trail_stop")

            if trend_down:
                self._in_position = False
                self._peak_price = 0.0
                return Signal(target_exposure=0.0, reason="vtrend_e5_ema21_d1_vc_trend_exit")
...
def _rolling_std(series: np.ndarray, window: int) -> np.ndarray:
    """Rolling sample standard deviation matching pandas rolling().std() (ddof=1)."""
    n = len(series)
