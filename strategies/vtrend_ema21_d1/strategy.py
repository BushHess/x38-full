"""E0_ema21D1 (vtrend_ema21_d1) — E0 + EMA(21) regime filter on D1 bars.

Same as E0 with an additional entry condition:
  D1 close > D1 EMA(d1_ema_period)

Unlike vtrend_ema21 which computes EMA(126) on H4 bars, this variant
computes EMA(21) on actual D1 bars and maps it to the H4 bar grid.
The D1 EMA updates once per day (not 6 times like H4).
...
class VTrendEma21D1Strategy(Strategy):
    def __init__(self, config: VTrendEma21D1Config | None = None) -> None:
        self._config = config or VTrendEma21D1Config()
    ...
    def on_bar(self, state: MarketState) -> Signal | None:
        ...
        if not self._in_position:
            ...
            if trend_up and vdo_val > self._c.vdo_threshold and regime_ok:
                ...
                return Signal(target_exposure=1.0, reason="vtrend_ema21_d1_entry")
        else:
            ...
            if price < trail_stop:
                self._in_position = False
                self._peak_price = 0.0
                return Signal(target_exposure=0.0, reason="vtrend_ema21_d1_trail_stop")

            if trend_down:
                self._in_position = False
                self._peak_price = 0.0
                return Signal(target_exposure=0.0, reason="vtrend_ema21_d1_trend_exit")
