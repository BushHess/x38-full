"""E0 (vtrend) — Volume-confirmed Trend strategy.

3-parameter BTC trend-following strategy designed for statistical validity.
Entry: EMA crossover + VDO confirmation (volume delta > 0).
Exit:  Trailing stop (ATR) OR EMA cross-down. No VDO in exit logic.
...
class VTrendStrategy(Strategy):
    def __init__(self, config: VTrendConfig | None = None) -> None:
        self._config = config or VTrendConfig()
    ...
    def on_bar(self, state: MarketState) -> Signal | None:
        ...
        if not self._in_position:
            # ENTRY: trend up AND VDO confirms buying pressure
            if trend_up and vdo_val > self._c.vdo_threshold:
                self._in_position = True
                self._peak_price = price
                return Signal(target_exposure=1.0, reason="vtrend_entry")
        else:
