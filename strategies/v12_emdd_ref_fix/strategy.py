"""V12 EMDD reference-fix strategy.

Step 2 objective: copy V10 baseline core logic into a separate V12 class line,
then gate emergency-DD reference selection via ``emdd_ref_mode``.
"""
...
# ---------------------------------------------------------------------------
# Configuration — V12 defaults copied from V10 baseline family
# ---------------------------------------------------------------------------

@dataclass
class V12EMDDRefFixConfig:
    """Parameters copied from V10/V8Apex with V12 EMDD reference mode switch."""
    ...
    emdd_ref_mode: str = "legacy"          # "legacy" | "fixed"

    def __post_init__(self) -> None:
        if self.emdd_ref_mode not in {"legacy", "fixed"}:
            raise ValueError(
                "emdd_ref_mode must be 'legacy' or 'fixed', "
                f"got {self.emdd_ref_mode!r}"
            )
...
class V12EMDDRefFixStrategy(Strategy):
    """V12 clone of V10 core logic with toggleable EMDD reference mode."""

    def __init__(self, cfg: V12EMDDRefFixConfig | None = None) -> None:
        self.cfg = cfg or V12EMDDRefFixConfig()
        # Cache mode once so config access is explicit and deterministic.
        self._emdd_ref_mode: str = str(self.cfg.emdd_ref_mode)
        # Tracking state
        ...
        self._emdd_ref_nav_true: float = 0.0
    ...
    def on_bar(self, state: MarketState) -> Signal | None:
        ...
        if just_opened:
            ...
            if self._emdd_ref_nav_true <= 0:
                # Fallback when engine does not provide on_after_fill hook.
                self._emdd_ref_nav_true = state.nav
    ...
    def on_after_fill(self, state: MarketState, fill: Fill) -> None:
        """Capture true entry NAV immediately after an opening BUY fill."""
        in_pos = state.btc_qty > 1e-8
        if fill.side == Side.BUY and in_pos and self._emdd_ref_nav_true <= 0:
            self._emdd_ref_nav_true = state.nav
        if fill.side == Side.SELL and not in_pos:
            self._emdd_ref_nav_true = 0.0
    ...
    def _check_exit(
        ...
        # 1. Emergency DD — reference mode switch:
        #   - legacy: match V10 pre-cost entry NAV parity path
        #   - fixed:  use true post-fill NAV captured by on_after_fill
        if self._emdd_ref_mode == "legacy":
            ref_nav = state.position_entry_nav
        else:
            ref_nav = self._emdd_ref_nav_true
            if ref_nav <= 0:
                # Defensive fallback for callers that bypass engine fill hooks.
                ref_nav = state.position_entry_nav
        if ref_nav > 0:
