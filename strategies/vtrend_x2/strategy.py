"""VTREND-X2 -- E0_ema21D1 with adaptive trailing stop.

Key change from E0_ema21D1 baseline: trail_mult is NOT fixed at 3.0 but adapts
based on unrealized gain of the current position.

Adaptive trailing stop logic:
  unrealized_gain = (close - entry_price) / entry_price

  if unrealized_gain < 0.05:
      trail_mult = trail_tight   (default 3.0)  -- protect capital early
  elif unrealized_gain < 0.15:
      trail_mult = trail_mid     (default 4.0)  -- loosen as buffer grows
  else:
      trail_mult = trail_wide    (default 5.0)  -- wide in strong trend

Rationale: 3xATR is too tight for crypto in trending markets (normal
pullbacks whipsaw out), but appropriate in choppy/early-trade conditions.
When a position has 15%+ unrealized gain, we're likely in a strong trend
and should widen the stop to ride it.

Entry logic: identical to E0_ema21D1 baseline.
Exit logic: adaptive trail stop OR EMA cross-down (same as E0).

Parameters (tunable -- 7 total):
  slow_period      -- EMA slow period (fast = max(5, slow // 4))
  trail_tight      -- ATR multiplier when unrealized_gain < gain_tier1
  trail_mid        -- ATR multiplier when gain_tier1 <= gain < gain_tier2
  trail_wide       -- ATR multiplier when unrealized_gain >= gain_tier2
  gain_tier1       -- threshold for switching tight -> mid (default 0.05)
  gain_tier2       -- threshold for switching mid -> wide (default 0.15)
...
STRATEGY_ID = "vtrend_x2"


@dataclass
class VTrendX2Config:
    # Tunable
    slow_period: float = 120.0
    trail_tight: float = 3.0     # unrealized_gain < gain_tier1
...
class VTrendX2Strategy(Strategy):
    def __init__(self, config: VTrendX2Config | None = None) -> None:
        self._config = config or VTrendX2Config()
    ...
    def _adaptive_trail_mult(self, price: float) -> float:
        """Return trail multiplier based on unrealized gain from entry."""
        if self._entry_price <= 0:
            return self._c.trail_tight
        unrealized_gain = (price - self._entry_price) / self._entry_price
        if unrealized_gain < self._c.gain_tier1:
            return self._c.trail_tight
        elif unrealized_gain < self._c.gain_tier2:
            return self._c.trail_mid
        else:
            return self._c.trail_wide
    ...
    def on_bar(self, state: MarketState) -> Signal | None:
        ...
        if not self._in_position:
            ...
            if trend_up and vdo_val > self._c.vdo_threshold and regime_ok:
                ...
                return Signal(target_exposure=1.0, reason="x2_entry")
        else:
            self._peak_price = max(self._peak_price, price)

            # EXIT: adaptive trailing stop OR trend reversal
            trail_mult = self._adaptive_trail_mult(price)
            trail_stop = self._peak_price - trail_mult * atr_val
            if price < trail_stop:
                self._in_position = False
                self._peak_price = 0.0
                self._entry_price = 0.0
                return Signal(target_exposure=0.0, reason="x2_trail_stop")

            if trend_down:
                self._in_position = False
                self._peak_price = 0.0
                self._entry_price = 0.0
                return Signal(target_exposure=0.0, reason="x2_trend_exit")
