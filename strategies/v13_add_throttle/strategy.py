"""V13 add-throttle strategy.

Copies V10 V8-Apex entry/exit logic and adds an overlays-only throttle
on add sizing based on rolling portfolio drawdown depth.
"""
...
class V13AddThrottleConfig:
    ...
    def __post_init__(self) -> None:
        if self.add_throttle_dd2 <= self.add_throttle_dd1:
            raise ValueError(
                "add_throttle_dd2 must be > add_throttle_dd1, "
                f"got dd1={self.add_throttle_dd1}, dd2={self.add_throttle_dd2}"
            )
        if not (0.0 <= self.add_throttle_mult <= 1.0):
            raise ValueError(
                "add_throttle_mult must be in [0, 1], "
                f"got {self.add_throttle_mult}"
            )
...
class V13AddThrottleStrategy(Strategy):
    """V13 add-throttle overlay on V8 Apex entry/exit logic."""

    def __init__(self, cfg: V13AddThrottleConfig | None = None) -> None:
        self.cfg = cfg or V13AddThrottleConfig()
        self._add_throttle_dd1 = float(self.cfg.add_throttle_dd1)
        self._add_throttle_dd2 = float(self.cfg.add_throttle_dd2)
        self._add_throttle_mult = float(self.cfg.add_throttle_mult)
        # Tracking state
        ...
        self._vdo_source: str = "taker_buy"
        ...
        # Add-throttle instrumentation
        self._add_attempt_count: int = 0
        self._add_allowed_count: int = 0
        self._add_blocked_count: int = 0
        self._blocked_dd_depths: list[float] = []
        ...
        if just_closed:
            ...
            if self._last_exit_reason == "emergency_dd":
                if self.cfg.escalating_cooldown:
                    ...
                    if (self._last_emergency_dd_bar_idx >= 0
                            and idx - self._last_emergency_dd_bar_idx
                            <= self.cfg.escalating_lookback_bars):
                        self._cascade_ed_count += 1
                    else:
                        self._cascade_ed_count = 1
                    self._last_emergency_dd_bar_idx = idx
                    if self._cascade_ed_count >= self.cfg.cascade_trigger_count:
                        self._emergency_dd_cooldown_remaining = (
                            self.cfg.long_cooldown_bars
                        )
                        self._active_cooldown_type = "long_cooldown"
                        self._cascade_ed_count = 0  # reset after trigger
                    else:
                        self._emergency_dd_cooldown_remaining = (
                            self.cfg.short_cooldown_bars
                        )
                        self._active_cooldown_type = "short_cooldown"
                else:
                    # V1: flat cooldown
                    self._emergency_dd_cooldown_remaining = (
                        self.cfg.cooldown_after_emergency_dd_bars
                    )
        if just_opened:
    ...
    def _add_cap_with_throttle(
        ...
        *,
        ...
        c = self.cfg
        if position_exposure <= 1e-8:
            # Flat: first entry must not be throttled.
            return c.max_add_per_bar
        if dd_depth >= self._add_throttle_dd2:
            return 0.0
        if dd_depth >= self._add_throttle_dd1:
            return c.max_add_per_bar * self._add_throttle_mult
        return c.max_add_per_bar

    def _record_add_blocked(self, dd_depth: float) -> None:
        self._add_blocked_count += 1
        self._blocked_dd_depths.append(float(dd_depth))

    def get_add_throttle_stats(self) -> dict[str, float | int]:
        attempts = int(self._add_attempt_count)
        allowed = int(self._add_allowed_count)
        blocked = int(self._add_blocked_count)
        blocked_depths = np.array(self._blocked_dd_depths, dtype=np.float64)
        mean_depth = float(np.mean(blocked_depths)) if blocked_depths.size > 0 else 0.0
        p90_depth = float(np.percentile(blocked_depths, 90)) if blocked_depths.size > 0 else 0.0
        return {
            "add_throttle_dd1": float(self._add_throttle_dd1),
            "add_throttle_dd2": float(self._add_throttle_dd2),
            "add_throttle_mult": float(self._add_throttle_mult),
            "add_attempt_count": attempts,
            "add_allowed_count": allowed,
            "add_blocked_count": blocked,
            "throttle_activation_rate": float(blocked / max(attempts, 1)),
            "mean_dd_depth_when_blocked": mean_depth,
            "p90_dd_depth_when_blocked": p90_depth,
        }
    ...
    def _check_entry(
        ...
        is_add_attempt = state.exposure > 1e-8
        if is_add_attempt:
            self._add_attempt_count += 1

        dd_depth = 0.0
        if self._equity_peak > 0 and state.nav > 0:
            dd_depth = max(0.0, 1.0 - (state.nav / self._equity_peak))
        allowed_add = self._add_cap_with_throttle(
            position_exposure=state.exposure,
            dd_depth=dd_depth,
        )
        if is_add_attempt and allowed_add <= 1e-12:
            self._record_add_blocked(dd_depth)
            return None
        ...
        raw_sz = min(sz, gap)
        sz = min(raw_sz, allowed_add)
        if sz < 0.01:
            if is_add_attempt and allowed_add < raw_sz:
                self._record_add_blocked(dd_depth)
            return None
