...
Behaviorally identical to E0_ema21D1.
Long-only trend-following with D1 EMA regime filter.
...
Exit (when long):
  1. Trail stop: close < peak_price - trail_mult * ATR(14)
...
class VTrendX0Strategy(Strategy):
    def __init__(self, config: VTrendX0Config | None = None) -> None:
        self._config = config or VTrendX0Config()
