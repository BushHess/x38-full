# PROMPT D1d - Walk-Forward Evaluation (Use this after D1c in the same chat)
...
**Performance note**: Prefer vectorized signal generation (e.g., pandas boolean columns and rolling computations) over bar-by-bar loops for speed. If the 15m CSV is too large, note that the constitution's execution model uses D1 and H4 as primary decision timeframes — 1h and 15m are for entry timing and execution refinement only.
...
- config_id, archetype, candidate_id, fold, cost_bps_rt, cagr, sharpe, max_drawdown, entries, exposure, mean_daily_return

Save aggregate results to `d1d_wfo_aggregate.csv` with columns:
- config_id, archetype, candidate_id, cost_bps_rt, agg_cagr, agg_sharpe, agg_mdd, total_entries, avg_exposure, mean_fold_sharpe, median_fold_sharpe
