...
The original `PROMPT_D1b/D1d/D1e/D1f` files are the first-generation split (now deprecated).
...
## Redesign freeze sequence

Use these three prompts in the **same chat**, in this exact order:
1. `PROMPT_R0_PRECHECK_NEW_SESSION.md`
2. `PROMPT_R1_REDESIGN_EXECUTION.md`
3. `PROMPT_R2_PACKAGE_STATE.md`

Use this sequence:
- only when a valid redesign trigger has fired,
- the cooldown period has elapsed,
- the evidence threshold is met,
- and a redesign dossier has been prepared.
...
- only after `governance_failure_dossier.md` has been prepared from `template/governance_failure_dossier.template.md`.

## Never do this

- D-sequence and F-sequence in one chat
- F-sequence and R-sequence in one chat
- R-sequence and G-sequence in one chat
