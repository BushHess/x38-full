...
def _build_feature_frame(data_by_timeframe: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    sig = _data_signature(data_by_timeframe, keys=["1d", "4h"], extra="c1")
    ...
    d1_range1 = (d1["high"] - d1["low"]) / d1["close"]
    d1["d1_rangevol84"] = _gap_rolling_mean(d1_range1, d1_seg, 84)
    d1["d1_rangevol84_rank365"] = _gap_pct_rank_inclusive(d1["d1_rangevol84"], d1_seg, 365)
    d1["d1_rangepos168"] = _gap_rangepos(d1["high"], d1["low"], d1["close"], d1_seg, 168)

    h4["h4_rangepos168"] = _gap_rangepos(h4["high"], h4["low"], h4["close"], h4_seg, 168)

    merged = h4.copy()
    d1_feats = _merge_asof_features(merged, d1, ["d1_rangevol84_rank365", "d1_rangepos168"])
    merged = pd.concat([merged, d1_feats], axis=1)
