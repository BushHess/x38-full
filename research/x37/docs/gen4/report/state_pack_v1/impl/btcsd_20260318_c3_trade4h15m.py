...
def _fit_trade_model(d1: pd.DataFrame, start_utc: Optional[Any]) -> Dict[str, float]:
    start_ts = _parse_utc_boundary(start_utc, is_end=False)
    if start_ts is not None:
        fit_mask = d1["dt_close"] < start_ts
    else:
        fit_mask = pd.Series(True, index=d1.index)
    fit_df = d1.loc[fit_mask].copy()
    if len(fit_df) < 2:
        fit_df = d1.iloc[: max(2, min(len(d1), 365))].copy()
    x = np.log1p(fit_df["volume"].to_numpy(dtype=float))
    y = np.log1p(fit_df["num_trades"].to_numpy(dtype=float))
    x_mean = float(np.mean(x))
    y_mean = float(np.mean(y))
    x_var = float(np.mean((x - x_mean) ** 2))
    beta = 0.0 if x_var <= 0 else float(np.mean((x - x_mean) * (y - y_mean)) / x_var)
    alpha = y_mean - beta * x_mean
    return {"alpha": alpha, "beta": beta}

def _build_feature_frame(data_by_timeframe: Dict[str, pd.DataFrame], start_utc: Optional[Any]) -> pd.DataFrame:
    sig = _data_signature(data_by_timeframe, keys=["1d", "4h", "15m"], extra=("c3", str(_parse_utc_boundary(start_utc, is_end=False))))
    ...
    m15 = _ensure_frame(data_by_timeframe["15m"], "15m")

    d1_seg = _segment_ids(d1["open_time"], _interval_ms_from_label("1d"))
    h4_seg = _segment_ids(h4["open_time"], _interval_ms_from_label("4h"))
    m15_seg = _segment_ids(m15["open_time"], _interval_ms_from_label("15m"))

    fit = _fit_trade_model(d1, start_utc=start_utc)
    x_all = np.log1p(d1["volume"].to_numpy(dtype=float))
    y_all = np.log1p(d1["num_trades"].to_numpy(dtype=float))
    eps = y_all - (fit["alpha"] + fit["beta"] * x_all)
    d1["d1_trade_surprise168"] = pd.Series(eps, index=d1.index) - _gap_rolling_mean(pd.Series(eps, index=d1.index), d1_seg, 168)

    h4["h4_rangepos168"] = _gap_rangepos(h4["high"], h4["low"], h4["close"], h4_seg, 168)
    m15["m15_relvol168"] = m15["volume"] / _gap_rolling_mean(m15["volume"], m15_seg, 168)

    merged = m15.copy()
    d1_feats = _merge_asof_features(merged, d1, ["d1_trade_surprise168"])
...
def _run_candidate_internal(
    ...
    disabled_layers = set(disabled_layers or [])
    config = _normalize_config(dict(config))
    df = _build_feature_frame(data_by_timeframe, start_utc=start_utc)

    d1_entry = pd.Series(True, index=df.index)
    d1_hold = pd.Series(True, index=df.index)
    if "d1_participation_permission" not in disabled_layers:
        d1_entry = df["d1_trade_surprise168"] > 0.0
    ...
    if "h4_context" not in disabled_layers:
        h4_entry = df["h4_rangepos168"] >= config["q_h4_rangepos_entry"]
        h4_hold = df["h4_rangepos168"] >= config["q_h4_rangepos_hold"]

    m15_entry = pd.Series(True, index=df.index)
    if "m15_timing" not in disabled_layers:
        m15_entry = df["m15_relvol168"] >= config["rho_m15_relvol_min"]

    df = df.copy()
    df["entry_cond"] = d1_entry & h4_entry & m15_entry
    df["hold_cond"] = d1_hold & h4_hold
