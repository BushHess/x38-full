# BTC/USDT Spot Long-Only System Discovery — Final Same-File Convergence Audit Protocol V7
...
This version is a **final same-file convergence audit**. Its purpose is to test whether a stricter protocol produces a stable, defensible internal result without importing prior-session specifics. Its purpose is **not** to imply that later same-file work is automatically better than earlier work, and it does **not** convert the current file into a source of clean new OOS evidence.
...
Methodological tightening can improve governance and auditability, but it does not by itself create new independent evidence. Do not treat prompt editing on the same file as a hidden substitute for new data.
...
Under V7, repeated same-file prompt revision is itself a search dimension. This protocol is intended to freeze that dimension for one final same-file audit rather than to encourage further same-file prompt tuning.
...
- Do not add new candidates, new features, new layers, new calibration modes, or new parameter cells after the holdout is opened.
...
- not available from the supplied file alone under V7;
...
3. stronger paired-bootstrap profile,
4. broader plateau,
...
- the Stage 1 feature-library manifest,
...
- treats a later same-file result as automatically more correct merely because it is later,
- uses repeated same-file prompt editing as a hidden optimization axis,
- fails to export the full scan manifest, anomaly-disposition register, and candidate ledger,
...
3. Three prior sessions have already been run and they produced different frozen outcomes. Treat none of them as privileged. The new session must follow its own evidence.
4. Do not try to reconcile prior outcomes or search “between” them. Treat the search space as open.
5. This session must use the split defined in this document. Do not revert to older splits.
6. The reserve window in this version is internal only and is not eligible for clean OOS.
7. If the new session converges on the same conclusion as earlier work, record that convergence explicitly after freeze.
8. If the new session diverges, do not treat earlier work as more correct. Follow the new evidence.
9. If no candidate clears the hard gates on discovery plus selection holdout, report **NO ROBUST IMPROVEMENT** rather than forcing a winner.
10. This protocol is the final same-file convergence audit for the current file. If it diverges again, record the divergence honestly and stop same-file prompt iteration instead of trying to repair the result by further same-file prompt tuning.

## Meta-knowledge from Prior Research

### New methodological lessons

- Later same-file sessions are not automatically better than earlier ones. Better governance can reduce optimistic selection bias and still produce a weaker headline result.
- Same-file prompt editing is itself a search dimension. Final same-file audit protocols should therefore be frozen and paired with an explicit stop condition.
- Divergent same-file winners are evidence about frontier instability or specification sensitivity. They should be recorded, not forcibly reconciled by construction.
- Internal reserve slices and later same-file runs are useful for contradiction testing, evidence grading, and process audit, but they do not by themselves create new independent proof.
