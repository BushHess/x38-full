# Hướng dẫn sử dụng Research Operating Kit v3 (First-Principles)
...
3. `PROMPT_D1b1_PRICE_MOMENTUM.md` — đo lường kênh giá/momentum
4. `PROMPT_D1b2_VOLATILITY_REGIME.md` — đo lường kênh biến động/regime
5. `PROMPT_D1b3_VOLUME_FLOW.md` — đo lường kênh khối lượng/order flow
6. `PROMPT_D1b4_CROSS_TF_RANKING.md` — cross-timeframe + redundancy + xếp hạng kênh
...
9. `PROMPT_D1d2_WFO_BATCH.md` — walk-forward batch (có thể gửi lại nếu timeout)
10. `PROMPT_D1d3_WFO_AGGREGATE.md` — tổng hợp metrics WFO
11. `PROMPT_D1e1_FILTER_SELECTION.md` — hard constraint filter + chọn config đại diện
12. `PROMPT_D1e2_HOLDOUT_RESERVE.md` — holdout + reserve evaluation
13. `PROMPT_D1e3_BOOTSTRAP_RANKING.md` — bootstrap + xếp hạng cuối cùng
14. `PROMPT_D1f1_FREEZE_SPECS.md` — freeze champion/challengers + frozen specs
15. `PROMPT_D1f2_REGISTRY_STATE.md` — registry + state files
16. `PROMPT_D1f3_AUDIT_LEDGER_MAP.md` — audit + ledger + contamination map
17. `PROMPT_D2_PACKAGE_STATE.md` — input_hash_manifest + package state_pack_v1
...
Nếu không có thay đổi kiểu đó, thì **giữ nguyên `research_constitution_v3.0.yaml`**.
