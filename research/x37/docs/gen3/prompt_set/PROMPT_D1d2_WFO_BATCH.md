...
### 1b. Run ablation variants (multi-layer candidates only)

For each ablation variant implemented in D1d1 (`d1d_impl_{candidate_id}_no_{layer_name}.py`):
- Run on **all 14 folds** at **50 bps RT only**, using the candidate's first config.
- Record the same per-fold metrics as step 1.
- Append results to `d1d_wfo_results.csv` with `config_id = {candidate_id}_no_{layer_name}`.
- Append daily returns to `d1d_wfo_daily_returns.csv` with the same config_id convention.

If no multi-layer candidates exist, skip this step.

### 2. Enforce runtime invariants
During execution, the implementation must assert the following invariants on every fold. If any invariant fails, stop and report the bug before continuing:
- **No same-bar fill**: entry and exit cannot occur on the same bar.
- **No overlapping positions**: at most one position open at any time.
- **Exposure bounded**: exposure ∈ [0, 1] on every bar.
- **Cost monotonicity**: net return ≤ gross return for every trade (cost cannot increase returns).
- **MTF alignment**: if the candidate uses cross-timeframe data, verify `slow.close_time < fast.close_time` per `TIMEFRAME_ALIGNMENT_SPEC_EN.md`.

These invariants run on ALL configs × folds, providing broader coverage than the D1d1 smoke test.
