...
**Save implementations to files**: Write each candidate's implementation to `d1d_impl_{candidate_id}.py`. Each file must be self-contained: it should define a function (or class) that takes data, config parameters, and cost as inputs and returns a daily returns series plus trade log. This file is the **named artifact** that D1d2, D1e2, and D1e3 will reload if runtime state is lost.
...
### 1b. Ablation variants (multi-layer candidates only)
