# PROMPT D1e - Holdout, Reserve & Ranking (DEPRECATED — use D1e1–D1e3)

> **Note**: This monolithic prompt has been split into 3 sub-prompts for better per-turn execution:
> - `PROMPT_D1e1_FILTER_SELECTION.md` — hard constraint filter + representative config selection + complexity penalty
> - `PROMPT_D1e2_HOLDOUT_RESERVE.md` — holdout + reserve_internal evaluation
> - `PROMPT_D1e3_BOOTSTRAP_RANKING.md` — bootstrap lower-bound check + final ranking
>
...
If **zero configs survive**, report this clearly. Still save `d1e_hard_constraint_filter.csv` (showing all configs failed) and empty `d1e_holdout_results.csv`, `d1e_reserve_results.csv`, `d1e_final_ranking.csv`. Then proceed to D1f — it will handle the `NO_ROBUST_CANDIDATE` path.
