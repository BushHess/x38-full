...
- `d1b_measurements.md` — **consolidated summary** integrating all D1b1–D1b4 results, containing:
  ...
  - **measurement audit trail**: total number of independent statistical tests performed across D1b1–D1b4, number of feature families tested, number of lookback variants per family, and estimated effective degrees of freedom (e.g. Nyholt M_eff or similar correction for correlated tests). This trail does not restrict exploration — it records the upstream search breadth so that D1c and downstream steps can assess the true DOF burden.
