#!/usr/bin/env python3
"""VBREAK — Donchian Breakout: Full 4-Phase Validation Pipeline.

4-phase early-stopping test against VTREND:
  Phase 1: Permutation test for breakout signal (10,000 perms)
  Phase 2: Timescale robustness (16 timescales × 2000 bootstrap paths)
  Phase 3: Paired bootstrap vs VTREND (2000 paths, head-to-head)
  Phase 4: Multiple comparison correction (Bonferroni/Holm/BH)

Entry: close > highest_high(N preceding bars) + VDO filter
Exit:  close < lowest_low(M preceding bars) OR ATR trailing stop

Parameters: N (entry lookback), M = N//3 (exit lookback), trail_mult
...
# Timescale grid (H4 bars) — same as VTREND
N_GRID = [30, 48, 60, 72, 84, 96, 108, 120, 144, 168, 200, 240, 300, 360, 500, 720]

# Exit channel ratio: M = max(5, N // M_RATIO)
M_RATIO = 3
...
def _lowest_low(low, m):
    """Rolling min of low[i-m:i]. NaN for i < m. No lookahead."""
...
# ═══════════════════════════════════════════════════════════════════════
# VBREAK fast simulation (incremental stats, same pattern as VTREND)
# ═══════════════════════════════════════════════════════════════════════

def sim_vbreak(cl, hh, ll, at, vd, wi, vdo_thr, trail=TRAIL):
    """VBREAK binary sim (f=1.0). Returns metrics dict."""
    n = len(cl)
    cash = CASH
    bq = 0.0
    inp = False
    ...
    for i in range(n):
        ...
        ll_val = ll[i]
        a_val = at[i]
        if math.isnan(hh_val) or math.isnan(ll_val) or math.isnan(a_val):
            continue

        if not inp:
            # ENTRY: close > highest_high(N) + VDO filter
            if p > hh_val and vd[i] > vdo_thr:
                pe = True
        else:
            pk = max(pk, p)
            # EXIT 1: close < lowest_low(M) — channel exit
            if p < ll_val:
                px = True
...
def composite_score(r):
    ...
    mdd = r["mdd"]
    sh = r["sharpe"]
    # Approximate profit factor from cagr and mdd
    # Use Sharpe as proxy since we don't track individual trade PnLs
...
def sim_vbreak_navs(cl, hh, ll, at, vd, wi, vdo_thr, trail=TRAIL):
    """VBREAK sim returning NAV array + trades (for score computation)."""
    n = len(cl)
    cash = CASH
    bq = 0.0
    inp = False
...
def run_permutation_breakout(cl, hi, lo, vo, tb, wi):
    """Test H0: breakout signal is random. Circular-shift hh/ll arrays."""
    print("\n" + "=" * 70)
    print("PHASE 1a: PERMUTATION TEST — BREAKOUT SIGNAL")
    print("=" * 70)

    N_def = 120
    M_def = max(5, N_def // M_RATIO)

    at = _atr(hi, lo, cl, ATR_P)
    vd = _vdo(cl, hi, lo, vo, tb, VDO_F, VDO_S)
    hh = _highest_high(hi, N_def)
    ll = _lowest_low(lo, M_def)

    real_r = sim_vbreak_navs(cl, hh, ll, at, vd, wi, VDO_ON)
    ...
    for i in range(N_PERM):
        ...
        ll_s = np.roll(ll, offset)

        r = sim_vbreak_navs(cl, hh_s, ll_s, at, vd, wi, VDO_ON)
...
def run_permutation_atr(cl, hi, lo, vo, tb, wi):
    ...
    for i in range(N_PERM):
        ...
        r = sim_vbreak_navs(cl, hh, ll, at_s, vd, wi, VDO_ON)
        null_scores[i] = full_score(r)
...
def run_real_data(cl, hi, lo, vo, tb, wi):
    ...
    for N in N_GRID:
        ...
        r_on  = sim_vbreak(cl, hh, ll, at, vd, wi, VDO_ON)
        r_off = sim_vbreak(cl, hh, ll, at, vd, wi, VDO_OFF)

        results[N] = {"with_vdo": r_on, "no_vdo": r_off}

        print(f"  {N:5d}  {M:4d}  {days:5.1f}  "
...
def run_bootstrap(cl, hi, lo, vo, tb, wi):
    ...
    print("\n" + "=" * 70)
    print(f"PHASE 2b: BOOTSTRAP {N_BOOT} PATHS × {len(N_GRID)} TIMESCALES × 2 VDO")
    print("=" * 70)
    ...
    for b in range(N_BOOT):
        ...
        for j, N in enumerate(N_GRID):
            ...
            r_on  = sim_vbreak(c, hh, ll, at, vd, wi, VDO_ON)
            r_off = sim_vbreak(c, hh, ll, at, vd, wi, VDO_OFF)
...
def analyze_timescale(boot_on, boot_off, real_results):
    """Analyze timescale robustness. Returns (prod_min, prod_max, strong_min, strong_max)."""
    n_sp = len(N_GRID)

    print("\n" + "=" * 70)
    print("VBREAK TIMESCALE ROBUSTNESS (WITH VDO)")
    print("=" * 70)
    ...
    for j, N in enumerate(N_GRID):
        ...
        print(f"  N={N:5d} ({days:5.1f}d)  "
              f"meanΔSh={d_sh.mean():+.4f}  P(VDO+)={p_helps*100:5.1f}%")
...
def run_paired_bootstrap(cl, hi, lo, vo, tb, wi, best_N):
    """2000 paths, VBREAK vs VTREND on same paths."""
    print("\n" + "=" * 70)
    print(f"PHASE 3: PAIRED BOOTSTRAP — VBREAK(N={best_N}) vs VTREND(slow={VT_SLOW})")
    print("=" * 70)

    best_M = max(5, best_N // M_RATIO)
    ...
    mkeys = ["cagr", "mdd", "sharpe", "calmar", "trades"]
    vb_boot = {m: np.zeros(N_BOOT) for m in mkeys}
    ...
    for b in range(N_BOOT):
        ...
        # VBREAK
        hh = _highest_high(h, best_N)
        ll = _lowest_low(l, best_M)
        rv = sim_vbreak(c, hh, ll, at, vd, wi, VDO_ON)
    ...
    return vb_boot, vt_boot


def analyze_paired(vb_boot, vt_boot):
    """Paired comparison analysis. Returns gate3_passed."""
    print("\n" + "-" * 70)
    print("BOOTSTRAP DISTRIBUTIONS")
    print("-" * 70)

    for lab, store in [("VBREAK", vb_boot), ("VTREND", vt_boot)]:
        print(f"\n  ── {lab} ──")
    ...
    print("-" * 70)
    print("  P(VBREAK better) > 97.5% ≈ significant at α=0.05 (one-sided).\n")
    ...
    for m, direction in [("cagr", "higher"), ("mdd", "lower"),
        ...
        if direction == "lower":
            d = vt_boot[m] - vb_boot[m]  # positive = VBREAK has lower MDD
        else:
            d = vb_boot[m] - vt_boot[m]  # positive = VBREAK is higher

        p_better = float(np.mean(d > 0))
        ci = np.percentile(d, [2.5, 97.5])
        sig = " ***" if p_better > 0.975 else " *" if p_better > 0.95 else ""

        print(f"    Δ{m:7s}  mean={d.mean():+8.4f}  "
              f"P(VBREAK {direction:6s})={p_better * 100:5.1f}%  "
              f"95%CI=[{ci[0]:+.4f}, {ci[1]:+.4f}]{sig}")

        paired_results[m] = {
            "mean_delta": round(float(d.mean()), 6),
            "p_vbreak_better": round(p_better, 4),
...
def run_correction(p_breakout, p_atr):
    ...
    # K=16 from VTREND (EMA, ATR, VDO, 6 regime gates, 6 sizing, V8)
    # +2 for VBREAK (breakout, ATR)
    # +1 for VPULL (already rejected)
    hyps = [
        ...
        # New VBREAK hypotheses
        ("VBREAK breakout signal",  p_breakout),
        ("VBREAK ATR trail",        p_atr),
    ]
    ...
    print(f"  Bonferroni threshold: α/K = {bonf:.6f}")
    print(f"\n  {'Hypothesis':<35s} {'p-value':>10s} {'Bonf':>6s} {'Holm':>6s} {'BH':>6s}")
    print("  " + "-" * 70)

    breakout_survives = {"bonferroni": False, "holm": False, "bh": False}
    ...
    for rank, (name, p) in enumerate(sorted_hyps):
        ...
        print(f"  {name:<35s} {p:10.6f} {'✓' if bonf_pass else '✗':>6s} "
              f"{'✓' if holm_pass else '✗':>6s} {'✓' if bh_pass else '✗':>6s}")

        if name == "VBREAK breakout signal":
            breakout_survives["bonferroni"] = bonf_pass
            breakout_survives["holm"] = holm_pass
            breakout_survives["bh"] = bh_pass

    print(f"\n  VBREAK breakout survives:")
    print(f"    Bonferroni: {'PASS' if breakout_survives['bonferroni'] else 'FAIL'}")
    print(f"    Holm:       {'PASS' if breakout_survives['holm'] else 'FAIL'}")
    print(f"    BH (FDR):   {'PASS' if breakout_survives['bh'] else 'FAIL'}")

    gate4_passed = breakout_survives["bonferroni"]
    print(f"\n  Gate 4: {'PASS' if gate4_passed else 'FAIL'} (Bonferroni)")

    return gate4_passed, breakout_survives, K
...
if __name__ == "__main__":
    print("=" * 70)
    print("VBREAK — DONCHIAN BREAKOUT: FULL VALIDATION PIPELINE")
    print("=" * 70)
    ...
    print(f"  Exit ratio: M = N // {M_RATIO}")
    ...
    p_break, real_sc, null_mean, null_std = run_permutation_breakout(cl, hi, lo, vo, tb, wi)
    p_atr, _, atr_null_mean, atr_null_std = run_permutation_atr(cl, hi, lo, vo, tb, wi)

    output["phase_1_permutation"] = {
        "breakout": {
            "p_value": round(p_break, 6),
    ...
    if p_break >= 0.001:
        ...
        output["fail_reason"] = f"Phase 1: breakout signal not significant (p={p_break:.6f})"
        outpath = outdir / "vbreak_test.json"
    ...
    vb_boot, vt_boot = run_paired_bootstrap(cl, hi, lo, vo, tb, wi, peak_N)
    gate3_passed, paired_results = analyze_paired(vb_boot, vt_boot)

    phase3 = {"best_N": peak_N, "best_M": max(5, peak_N // M_RATIO)}
    for lab, store in [("vbreak", vb_boot), ("vtrend", vt_boot)]:
        phase3[lab] = {}
        for m in ["cagr", "mdd", "sharpe", "calmar"]:
            a = store[m]
    ...
    if not gate3_passed:
        output["determination"] = "FAIL"
        output["fail_reason"] = "Phase 3: VBREAK not significantly better than VTREND"
        outpath = outdir / "vbreak_test.json"
        with open(outpath, "w") as f:
            json.dump(output, f, indent=2, default=str)
    ...
    gate4_passed, survives, K = run_correction(p_break, p_atr)

    output["phase_4_correction"] = {
        "K": K,
        "alpha": 0.05,
        "bonferroni_threshold": round(0.05 / K, 6),
        "breakout_survives_bonferroni": survives["bonferroni"],
        "breakout_survives_holm": survives["holm"],
        "breakout_survives_bh": survives["bh"],
    }
    ...
    if gate4_passed:
        ...
        output["determination"] = "FAIL"
        output["fail_reason"] = "Phase 4: breakout does not survive Bonferroni"
