...
> Status: CONVERGED (debate closed C.1→C.4, 12/12 items, 0 open)
> Evidence: `tmp/trilateral_debate_protocol.md` (debate artifact, read-only)
> Participants: claude_code, chatgpt_pro (bilateral debate, 4 rounds)
...
### D6. Provenance Metadata

Every round artifact from a participant WITHOUT direct file-system
access (currently: chatgpt_pro, codex when running outside CLI)
MUST include a metadata block:

```yaml
source: chatgpt_pro          # agent identifier
...
Step 1 — Opening:
  ...
  Architect writes: {architect}/round-1_opening-critique.md
  Human saves artifact to repo (§25b: human = single writer)

Step 2 — Round 1 rebuttal:
  IF N ≥ 3 (parallel):
    Human → Reviewer 1: Prompt B1 (WITHOUT Reviewer 2 artifact)
    Human → Reviewer 2: Prompt B2 (WITHOUT Reviewer 1 artifact)
    (concurrent — do not wait for each other)
    Human saves both artifacts
  IF N = 2 (bilateral):
...
debate/{domain}/
  ...
  └── external/{source}/*.md            ← advisory lane (optional)
```

Participant directories created only when debate starts.
Topics CLOSED before protocol upgrade keep bilateral structure.
