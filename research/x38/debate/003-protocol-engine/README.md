...
- **Upstream**: Topic 001 (campaign model) + Topic 002 (contamination firewall)
  ...
  + Topic 017A (intra-campaign ESP v1 — cell-elite archive, descriptor tagging,
    epistemic_delta.json ảnh hưởng pipeline stage design; phải CLOSED trước 003.
    017B v2 contracts can run parallel with 003.)
  + Topic 019A (discovery foundations) + Topic 019D1 (DFL-10 Stage 2.5 proposal —
    phải CLOSED trước 003. Other 019 subs are informational, not blocking.)
...
| 017A | ESP-01 | Cell-elite archive replaces Stage 4 global pruning. Descriptor tagging adds Stage 3 output. epistemic_delta.json adds Stage 8 mandatory output. Local probes change Stage 5 search strategy. | 003 owns pipeline structure; 017A defines v1 ESP contracts. 017A must CLOSE before 003 debate. Post-017A: 013 deferred numerics reconciled here. |
...
| 019A/019D1 | DFL-10 (019D1) | Discovery loop may add human-feedback checkpoints or AI analysis hooks between/after pipeline stages. **DFL-10 proposes Stage 2.5** (Data Characterization) between Stages 2-3, or expanding Stage 2 scope. 019A (foundations) provides architectural scope. | 003 owns pipeline structure and stage count. DFL-10 proposes; 003 decides: accept Stage 2.5, fold into Stage 2, or reject. 019A + 019D1 must CLOSE before 003 debate. |
