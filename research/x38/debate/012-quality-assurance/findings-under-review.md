...
2. **Cost escalation**: Bug phát hiện muộn trong module nền tảng (engine, data
   alignment) → chi phí tăng theo số module phụ thuộc (Boehm 1981). Module
   review gate phát hiện bug sớm tại tầng thấp → chi phí = 0 cho tầng trên.
...
- v10/tests/test_engine_invariants.py [extra-archive]: representative of ~370 tests trong v10 — precedent cho engine testing
- validation/tests/test_acceptance.py [extra-archive]: representative of 22 test modules cho validation pipeline
- research/x37/resource/gen1/v8_sd1trebd/spec/spec_2_system_S_D1_TREND.md [extra-archive]: bit-level system spec với test vectors — pattern cho engine testing
