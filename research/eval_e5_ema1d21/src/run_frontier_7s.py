#!/usr/bin/env python3
"""6-Strategy Matched-Risk Frontier (with E5_plus_EMA1D21).

Strategies: E0, E5, SM, LATCH, E0_plus_EMA1D21, E5_plus_EMA1D21
...
def main():
    print("6-Strategy Matched-Risk Frontier (with E5_plus_EMA1D21)")
    print("=" * 70)
    t0 = time.time()

    # ── 1. Load equity curves from factorial_7s ──────────────────────────
    npz = np.load(str(ARTIFACTS / "factorial_7s_equity_curves.npz"))
    curves = {k: npz[k] for k in npz.files}

    with open(ARTIFACTS / "step3_7s_master_results.json") as f:
        step3 = json.load(f)
    ...
    for name, s in strategies.items():
        ...
        print(f"  {short:<20} {status}  Sharpe_rng={lc['sharpe_pct_range']:.1f}%  "
    ...
    for name, s in strategies.items():
        ...
        short = name.replace("_Native", "")
        print(f"  {short:<20} k=0.50: CAGR={r50['cagr']:.4f} MDD={r50['mdd']:.4f} "
    ...
    for name, mdd in matched["native_mdds"].items():
        short = name.replace("_Native", "")
        print(f"    {short:<20} MDD={mdd:.4f} ({mdd*100:.2f}%)")
    ...
    pd.DataFrame(all_frontier_rows).to_csv(ARTIFACTS / "frontier_7s_grid.csv", index=False)
    print(f"  frontier_7s_grid.csv ({len(all_frontier_rows)} rows)")

    # Linearity check
    with open(ARTIFACTS / "linearity_7s_check.json", "w") as f:
        json.dump({k: _convert_dict(v) for k, v in linearity.items()}, f, indent=2)

    # Matched MDD
    if matched.get("matched_table"):
        pd.DataFrame(matched["matched_table"]).to_csv(ARTIFACTS / "matched_mdd_7s.csv", index=False)

    # Risk budget
    pd.DataFrame(risk_budget).to_csv(ARTIFACTS / "risk_budget_7s.csv", index=False)
    ...
    with open(ARTIFACTS / "step4_7s_master_results.json", "w") as f:
