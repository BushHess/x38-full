#!/usr/bin/env python3
"""6-Strategy Statistical Robustness + Temporal Stability.

Extends run_robustness.py Step 5 from 4 to 6 strategies.

Hypotheses (all at block lengths 42, 126, 252; 5000 reps):
  H1: Sharpe(E0_EV12)     > Sharpe(E0_plus_EMA1D21_EV12)   [best vs promoted]
  H2: Sharpe(E0_EV12)     > Sharpe(E5_EV12)          [baseline vs robust ATR]
  H3: Sharpe(E0_plus_EMA1D21_EV12) > Sharpe(SM_EV12)        [promoted vs SM]
  H4: Sharpe(E0_plus_EMA1D21_EV12) > Sharpe(LATCH_EV12)     [promoted vs LATCH]
  H5: Sharpe(E0_plus_EMA1D21_EV12) > Sharpe(EMA21_H4_EV12)  [D1 vs H4 regime]
  H6: Sharpe(E5_EV12)     > Sharpe(E0_EV12)          [robust ATR edge?]
  H7: Sharpe(LATCH_native) > Sharpe(SM_native)        [complexity premium]
...
def main():
    print("6-Strategy Statistical Robustness + Temporal Stability")
    print("=" * 70)
    t0 = time.time()
    ...
    for i, a in enumerate(STRATEGY_NAMES):
        for j, b in enumerate(STRATEGY_NAMES):
            ...
            print(f"  {a:>10} - {b:<10}: mean={res['mean']:+.4f}  "
    ...
    for i, res in enumerate(all_pairs):
        print(f"    {res['pair']:>25}: raw_p={raw_pvals_pairs[i]:.4f}  "
    ...
    hypotheses = [
        ...
        ("H5", "E0_plus_EMA1D21_EV12 > EMA21_H4_EV12", "E0_plus_EMA1D21_EntryVol_12", "EMA21_H4_EntryVol_12"),
        ("H6", "E5_EV12 > E0_EV12",          "E5_EntryVol_12",       "E0_EntryVol_12"),
        ("H7", "LATCH_native > SM_native",    "LATCH_Native",         "SM_Native"),
    ]
    ...
    for label, start_ms in start_dates_ms.items():
        ...
        for sname in STRATEGY_NAMES:
            ...
            row[f"{sname}_cagr"] = c
            row[f"{sname}_mdd"] = m
            print(f"    {sname:>10}: Sharpe={sh:.4f}  CAGR={c:.4f}  MDD={m:.4f}")
    ...
    for window_months in [24, 36]:
        ...
        for sname in STRATEGY_NAMES:
            w = wins.get(sname, 0)
            print(f"    {sname:>10} wins Sharpe: {w}/{n_windows} ({w/max(n_windows,1)*100:.0f}%)")
    ...
    for yr in years:
        ...
        print(f"  {yr:>6}", end="")
        for sname in STRATEGY_NAMES:
            print(f" {row.get(f'{sname}_sharpe', 0):>10.3f}", end="")
    ...
    pd.DataFrame(pairs_clean).to_csv(ARTIFACTS / "bootstrap_6s_all_pairs.csv", index=False)
    ...
    pd.DataFrame(named_clean).to_csv(ARTIFACTS / "bootstrap_6s_named_hypotheses.csv", index=False)

    # Method sensitivity
    pd.DataFrame(method_sensitivity).to_csv(ARTIFACTS / "bootstrap_6s_method_sensitivity.csv", index=False)

    # Matched MDD
    matched_clean = [_clean_row(r) for r in matched_results]
    pd.DataFrame(matched_clean).to_csv(ARTIFACTS / "bootstrap_6s_matched_mdd.csv", index=False)

    # Temporal
    pd.DataFrame(sds_rows).to_csv(ARTIFACTS / "start_date_6s.csv", index=False)
    pd.DataFrame(rw_rows).to_csv(ARTIFACTS / "rolling_window_6s.csv", index=False)
    pd.DataFrame(cal_rows).to_csv(ARTIFACTS / "calendar_slice_6s.csv", index=False)
    ...
    with open(ARTIFACTS / "holm_6s_pvalues.json", "w") as f:
    ...
    with open(ARTIFACTS / "step5_6s_master_results.json", "w") as f:
