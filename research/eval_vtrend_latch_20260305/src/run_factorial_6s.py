#!/usr/bin/env python3
"""6-Strategy Factorial: E0, E5, SM, LATCH, EMA21-H4, E0_plus_EMA1D21.

Extends run_factorial.py Step 3 from 4 to 6 strategies:
  ...
  - EMA21-H4: E0 + close > EMA(126, H4) regime filter (21d * 6 bars/d = 126)
...
def compute_indicators(bars: list[Bar], d1_data: dict | None = None,
    ...
    # EMA regime for EMA21-H4 (21 days × 6 bars/day = 126 H4 bars)
...
def extract_ema21_h4_signal(ind: dict, trail_mult: float = 3.0,
                             vdo_threshold: float = 0.0) -> dict:
    """EMA21-H4 binary signal: E0 + close > EMA(126, H4) regime filter."""
    n = len(ind["close"])
    close = ind["close"]
    ema_f, ema_s = ind["ema_fast"], ind["ema_slow"]
    atr_arr, vdo_arr = ind["atr"], ind["vdo"]
    ema_r = ind["ema_regime_h4"]
    ...
    for i in range(n):
        if i < 1 or np.isnan(atr_arr[i]) or np.isnan(ema_f[i]) or np.isnan(ema_s[i]) or np.isnan(ema_r[i]):
            in_pos[i] = is_in
            continue

        if not is_in:
            if ema_f[i] > ema_s[i] and vdo_arr[i] > vdo_threshold and close[i] > ema_r[i]:
                is_in = True
                peak = close[i]
                entry[i] = True
        else:
...
def signal_concordance(signals: dict[str, dict]) -> pd.DataFrame:
    """Compute 6x6 concordance matrix (% bars agreeing on position)."""
...
def main():
    print("6-Strategy Factorial: E0, E5, SM, LATCH, EMA21-H4, E0_plus_EMA1D21")
    print("=" * 70)
    t0 = time.time()
    ...
    print("Indicators computed (including robust ATR, EMA regime H4, D1 regime)")
    ...
    signals = {
        ...
        "EMA21_H4":  extract_ema21_h4_signal(ind),
    ...
    for sname, sig in signals.items():
        ...
        print(f"  {sname:>10}: {ent:>4} entries, {ext:>4} exits, in-pos {ip_pct:.1f}%")
    ...
    conc.to_csv(ARTIFACTS / "concordance_6s.csv")
    ...
    print("=" * 70)
    print(f"  {'Signal':<12} {'CAGR%':>8} {'MDD%':>8} {'Sharpe':>8} "
          f"{'PF':>6} {'Trades':>7} {'Score':>8}")
    print("  " + "-" * 65)
    for sname in STRATEGY_NAMES:
        m = factorial[f"{sname}_Binary_100"]
        print(f"  {sname:<12} {m['cagr']*100:>8.2f} {m['mdd']*100:>8.2f} "
              f"{m['sharpe']:>8.4f} {m['profit_factor']:>6.2f} "
              f"{int(m['n_trade_events']):>7} {m['score']:>8.2f}")

    print(f"\n{'NATIVE REFERENCES':}")
    print(f"  {'Strategy':<12} {'CAGR%':>8} {'MDD%':>8} {'Sharpe':>8} "
...
def _save_artifacts(factorial, native, pf_result, signals, ind, elapsed, conc):
    ...
    for name, m in all_runs.items():
        ...
        rows.append(row)
    pd.DataFrame(rows).to_csv(ARTIFACTS / "factorial_6s_summary.csv", index=False)
    ...
    pd.DataFrame(binary_rows).to_csv(
        ARTIFACTS / "signal_comparison_6s_binary100.csv", index=False)

    # 3. Equity curves (npz)
    eq_dict = {}
    for name, m in all_runs.items():
        eq_dict[name] = m["_eq"]
    np.savez_compressed(str(ARTIFACTS / "factorial_6s_equity_curves.npz"), **eq_dict)
    ...
    with open(ARTIFACTS / "step3_6s_master_results.json", "w") as f:
