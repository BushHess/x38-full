...
Adds trendq_84 > threshold to E5-ema21D1 entry.
Sweeps threshold in [0.0, 0.2, 0.4, 0.6, 0.8] + baseline (no gate).

Usage:
    python -m research.x39.experiments.exp02_trendq_gate
    # or from x39/:
    python experiments/exp02_trendq_gate.py
"""
...
def run_backtest(
    ...
    """Replay E5-ema21D1 with optional trendq gate. Returns summary dict."""
    c = feat["close"].values
    ...
    for i in range(warmup_bar, n):
        ...
        if not in_pos:
            ...
            if trendq_threshold is not None:
                entry_ok = entry_ok and np.isfinite(trendq[i]) and trendq[i] > trendq_threshold
    ...
    if len(eq) < 2 or len(trades) == 0:
        return {
            "threshold": trendq_threshold if trendq_threshold is not None else "baseline",
...
def main() -> None:
    ...
    print("=" * 80)
    print("EXP 02: Trend Quality Entry Gate")
    print("=" * 80)

    h4, d1 = load_data()
    feat = compute_features(h4, d1)

    # Warmup: 120 H4 bars for EMA slow, 84 for trendq
    ...
    # Save
    out_path = RESULTS_DIR / "exp02_results.csv"
    ...
    if not improvements.empty:
        ...
        if not sharpe_up.empty:
            ...
            print("Trend quality gate does NOT help E5-ema21D1.")
