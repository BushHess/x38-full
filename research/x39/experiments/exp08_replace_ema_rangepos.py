...
Replaces EMA fast/slow crossover entry AND EMA cross-down exit with rangepos_168.
Entry: rangepos_168 > entry_thresh  (price in upper portion of 168-bar range)
Exit:  close < trail_stop OR rangepos_168 < exit_thresh

Hysteresis: entry_thresh > exit_thresh to avoid whipsawing.

Sweeps entry_thresh in [0.50, 0.55, 0.65] × exit_thresh in [0.30, 0.40] + baseline.

Usage:
    python -m research.x39.experiments.exp08_replace_ema_rangepos
    # or from x39/:
    python experiments/exp08_replace_ema_rangepos.py
"""
...
def run_rangepos(
    ...
    """Modified: rangepos_168 > entry_thresh entry, rangepos_168 < exit_thresh exit."""
    c = feat["close"].values
    ratr = feat["ratr"].values
    vdo_arr = feat["vdo"].values
    d1_ok = feat["d1_regime_ok"].values
    ...
    for i in range(warmup_bar, n):
        if np.isnan(ratr[i]) or np.isnan(rp[i]):
            equity[i] = cash if not in_pos else position_size * c[i]
            continue

        if not in_pos:
            equity[i] = cash
            entry_ok = rp[i] > entry_thresh and vdo_arr[i] > 0 and d1_ok[i]
            if entry_ok:
                in_pos = True
                entry_bar = i
                entry_price = c[i]
            ...
            if c[i] < trail_stop:
                exit_reason = "trail"
            elif rp[i] < exit_thresh:
                exit_reason = "trend_rangepos"
    ...
    return _compute_metrics(equity, trades, warmup_bar,
                            f"entry={entry_thresh}_exit={exit_thresh}")
...
def main() -> None:
    ...
    print("=" * 80)
    print("EXP 08: Replace EMA Crossover with rangepos_168")
    print("=" * 80)
    ...
    rp = feat["rangepos_168"].values
    valid_count = np.isfinite(rp).sum()
    print(f"rangepos_168: {valid_count}/{len(rp)} valid bars")
    valid_vals = rp[np.isfinite(rp)]
    if len(valid_vals) > 0:
        ...
        for th in ENTRY_THRESHOLDS:
            ...
            print(f"  Bars with rangepos_168 > {th} (entry allowed): {pct:.1f}%")
    ...
    warmup_bar = max(SLOW_PERIOD, first_valid_rp, 168)
    print(f"Warmup bar: {warmup_bar} (rangepos_168 first valid at {first_valid_rp})")
    ...
    # ── Run rangepos variants ─────────────────────────────────────────
    for entry_th in ENTRY_THRESHOLDS:
        for exit_th in EXIT_THRESHOLDS:
            label = f"entry={entry_th}_exit={exit_th}"
            print(f"\nRunning {label}...")
            r = run_rangepos(feat, entry_th, exit_th, warmup_bar)
            results.append(r)
    ...
    out_path = RESULTS_DIR / "exp08_results.csv"
    ...
    if not improvements.empty:
        ...
        if not sharpe_up.empty:
            ...
            print("FAIL: No rangepos_168 config improves Sharpe over baseline.")
            print("rangepos_168 does NOT work as EMA crossover replacement for E5-ema21D1.")
