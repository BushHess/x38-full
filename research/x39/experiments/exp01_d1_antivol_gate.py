...
Adds d1_rangevol84_rank365 < threshold to E5-ema21D1 entry.
Sweeps threshold in [0.30, 0.40, 0.50, 0.60, 0.70] + baseline (no gate).

Usage:
    python -m research.x39.experiments.exp01_d1_antivol_gate
    # or from x39/:
    python experiments/exp01_d1_antivol_gate.py
"""
...
def run_backtest(
    feat: pd.DataFrame,
    antivol_threshold: float | None,
    warmup_bar: int,
) -> dict:
    """Replay E5-ema21D1 with optional anti-vol gate. Returns summary dict."""
    c = feat["close"].values
    ...
    cash = INITIAL_CASH
    position_size = 0.0  # units of BTC
    bars_in_position = 0
    ...
    for i in range(warmup_bar, n):
        ...
        if not in_pos:
            ...
            if antivol_threshold is not None:
                entry_ok = entry_ok and np.isfinite(rank365[i]) and rank365[i] < antivol_threshold
...
def main() -> None:
    ...
    print("=" * 80)
    print("EXP 01: D1 Anti-Vol Entry Gate")
    print("=" * 80)

    h4, d1 = load_data()
    feat = compute_features(h4, d1)

    # Warmup: 365 D1 bars for rank365 + 120 H4 bars for EMA slow
    # Find first H4 bar where d1_rangevol84_rank365 is valid
    ...
    # Save
    out_path = RESULTS_DIR / "exp01_results.csv"
    ...
    # Check if any threshold improves Sharpe AND MDD simultaneously
    gated = df.iloc[1:]
    ...
    if not improvements.empty:
        ...
        if not sharpe_up.empty:
            ...
            print("D1 anti-vol gate does NOT help E5-ema21D1.")
