#!/usr/bin/env python3
"""VCUSUM — Sequential Change-Point Detection: Full 4-Phase Validation Pipeline.

4-phase early-stopping test against VTREND:
  Phase 1: Permutation test for CUSUM signal (10,000 perms)
...
Entry: Upward CUSUM(z-scored log returns) > threshold + VDO filter
Exit:  Downward CUSUM > threshold OR ATR trailing stop

Parameters: ref_window (z-score reference), threshold, trail_mult
Total: 3 free params (same as VTREND)
...
VDO_S   = 28
TRAIL   = 3.0
CUSUM_K = 0.5    # Allowance (optimal for 1-sigma shift detection)
THRESH  = 4.0    # Default CUSUM threshold

# VDO thresholds
VDO_ON  = 0.0     # standard: VDO > 0 required
VDO_OFF = -1e9    # disabled: always passes

# Timescale grid (H4 bars) — same as VTREND/VBREAK for fair comparison
# ref_window controls the z-scoring reference period
W_GRID = [30, 48, 60, 72, 84, 96, 108, 120, 144, 168, 200, 240, 300, 360, 500, 720]
...
def _rolling_zscore(returns, window):
    """Z-score returns using trailing reference window (vectorized).

    z[i] = (r[i] - mean(r[i-W:i])) / std(r[i-W:i], ddof=1)
    z[i] = 0 for i < window.

    Uses cumulative sums for O(n) instead of O(n*W).
    """
    n = len(returns)
    ...
    # Cumulative sums for rolling stats
    cs = np.empty(n + 1, dtype=np.float64)
    cs2 = np.empty(n + 1, dtype=np.float64)
    cs[0] = 0.0
    cs2[0] = 0.0
    np.cumsum(returns, out=cs[1:])
    np.cumsum(returns ** 2, out=cs2[1:])

    w = window
    # For bar i (i >= w): reference = r[i-w:i] (w elements preceding bar i)
    # sum(r[i-w:i]) = cs[i] - cs[i-w]
    s = cs[w:n] - cs[:n - w]           # shape: (n - w,)
    s2 = cs2[w:n] - cs2[:n - w]

    mu = s / w
    var = (s2 - s * s / w) / (w - 1)   # Bessel-corrected
    sigma = np.sqrt(np.maximum(var, 0.0))

    valid = sigma > 1e-12
    z[w:n] = np.where(valid, (returns[w:n] - mu) / sigma, 0.0)
    return z


def _cusum(z, k):
    """Two-sided Page's CUSUM.

    cusum_up detects positive mean shift, cusum_dn detects negative.
    """
    n = len(z)
...
# ═══════════════════════════════════════════════════════════════════════
# VCUSUM fast simulation (incremental stats, same pattern as VTREND)
# ═══════════════════════════════════════════════════════════════════════

def sim_vcusum(cl, cup, cdn, at, vd, wi, threshold, vdo_thr, trail=TRAIL):
    """VCUSUM binary sim (f=1.0). Returns metrics dict."""
    n = len(cl)
    cash = CASH
    bq = 0.0
    inp = False
    ...
    for i in range(n):
        ...
        if not inp:
            # ENTRY: upward CUSUM > threshold + VDO confirms
            if cup[i] > threshold and vd[i] > vdo_thr:
                pe = True
        else:
            pk = max(pk, p)
            # EXIT 1: downward CUSUM detects negative shift
            if cdn[i] > threshold:
                px = True
            # EXIT 2: ATR trailing stop
            elif p < pk - trail * a_val:
                px = True
...
def sim_vcusum_navs(cl, cup, cdn, at, vd, wi, threshold, vdo_thr, trail=TRAIL):
    """VCUSUM sim returning NAV array + trades (for score computation)."""
    n = len(cl)
    cash = CASH
    bq = 0.0
    inp = False
...
def run_permutation_cusum(cl, hi, lo, vo, tb, wi):
    """Test H0: CUSUM signal is random. Circular-shift cusum arrays."""
    print("\n" + "=" * 70)
    print("PHASE 1a: PERMUTATION TEST — CUSUM SIGNAL")
    print("=" * 70)

    at = _atr(hi, lo, cl, ATR_P)
    vd = _vdo(cl, hi, lo, vo, tb, VDO_F, VDO_S)
    log_ret = _log_returns(cl)
    z = _rolling_zscore(log_ret, 120)  # default ref_window
    cup, cdn = _cusum(z, CUSUM_K)

    real_r = sim_vcusum_navs(cl, cup, cdn, at, vd, wi, THRESH, VDO_ON)
    ...
    for i in range(N_PERM):
        ...
        cup_s = np.roll(cup, offset)
        cdn_s = np.roll(cdn, offset)

        r = sim_vcusum_navs(cl, cup_s, cdn_s, at, vd, wi, THRESH, VDO_ON)
...
def run_permutation_atr(cl, hi, lo, vo, tb, wi):
    ...
    for i in range(N_PERM):
        ...
        r = sim_vcusum_navs(cl, cup, cdn, at_s, vd, wi, THRESH, VDO_ON)
        null_scores[i] = full_score(r)
...
def run_real_data(cl, hi, lo, vo, tb, wi):
    """Sweep ref_window timescales on real data, with and without VDO."""
    print("\n" + "=" * 70)
    ...
    for W in W_GRID:
        ...
        z = _rolling_zscore(log_ret, W)
        cup, cdn = _cusum(z, CUSUM_K)

        r_on  = sim_vcusum(cl, cup, cdn, at, vd, wi, THRESH, VDO_ON)
        r_off = sim_vcusum(cl, cup, cdn, at, vd, wi, THRESH, VDO_OFF)

        results[W] = {"with_vdo": r_on, "no_vdo": r_off}

        print(f"  {W:5d}  {days:5.1f}  "
...
def run_bootstrap(cl, hi, lo, vo, tb, wi):
    ...
    print("\n" + "=" * 70)
    print(f"PHASE 2b: BOOTSTRAP {N_BOOT} PATHS x {len(W_GRID)} TIMESCALES x 2 VDO")
    print("=" * 70)
    ...
    for b in range(N_BOOT):
        ...
        log_ret = _log_returns(c)

        for j, W in enumerate(W_GRID):
            z = _rolling_zscore(log_ret, W)
            cup, cdn = _cusum(z, CUSUM_K)

            r_on  = sim_vcusum(c, cup, cdn, at, vd, wi, THRESH, VDO_ON)
            r_off = sim_vcusum(c, cup, cdn, at, vd, wi, THRESH, VDO_OFF)
...
def analyze_timescale(boot_on, boot_off, real_results):
    """Analyze timescale robustness. Returns (prod_min, prod_max, ...)."""
    n_sp = len(W_GRID)

    print("\n" + "=" * 70)
    print("VCUSUM TIMESCALE ROBUSTNESS (WITH VDO)")
    print("=" * 70)
    ...
    if productive_min is not None and productive_max is not None:
        ...
        print(f"    Productive region: W={productive_min}-{productive_max} "
    ...
    if strong_min is not None and strong_max is not None:
        ...
        print(f"    Strong region:     W={strong_min}-{strong_max} "
    ...
    for j, W in enumerate(W_GRID):
        days = W * 4 / 24
        r_on = real_results[W]["with_vdo"]
        pct = np.mean(boot_on["sharpe"][:, j] <= r_on["sharpe"]) * 100
        flag = " !" if pct > 97.5 else " ?" if pct > 95 else ""
        print(f"  W={W:5d}  realSh={r_on['sharpe']:+.3f}  pctile={pct:.1f}%{flag}")

    # Find peak timescale
    peak_j = int(np.argmax(med_sharpes))
    peak_W = W_GRID[peak_j]
    print(f"\n  Peak median Sharpe: {med_sharpes[peak_j]:.4f} at W={peak_W} "
          f"({peak_W * 4 / 24:.0f}d)")

    return productive_min, productive_max, strong_min, strong_max, peak_W, width
...
def run_paired_bootstrap(cl, hi, lo, vo, tb, wi, best_W):
    """2000 paths, VCUSUM vs VTREND on same paths."""
    print("\n" + "=" * 70)
    print(f"PHASE 3: PAIRED BOOTSTRAP — VCUSUM(W={best_W}) vs VTREND(slow={VT_SLOW})")
    print("=" * 70)
    ...
    mkeys = ["cagr", "mdd", "sharpe", "calmar", "trades"]
    vc_boot = {m: np.zeros(N_BOOT) for m in mkeys}
    ...
    for b in range(N_BOOT):
        ...
        # VCUSUM
        log_ret = _log_returns(c)
        z = _rolling_zscore(log_ret, best_W)
        cup, cdn = _cusum(z, CUSUM_K)
        rv = sim_vcusum(c, cup, cdn, at, vd, wi, THRESH, VDO_ON)

        # VTREND
        ef = _ema(c, VT_FAST)
        es = _ema(c, VT_SLOW)
        rt = sim_vtrend(c, ef, es, at, vd, wi, VDO_ON)
    ...
    return vc_boot, vt_boot


def analyze_paired(vc_boot, vt_boot):
    """Paired comparison analysis. Returns gate3_passed."""
    print("\n" + "-" * 70)
    print("BOOTSTRAP DISTRIBUTIONS")
    print("-" * 70)

    for lab, store in [("VCUSUM", vc_boot), ("VTREND", vt_boot)]:
        print(f"\n  -- {lab} --")
    ...
    print("-" * 70)
    print("  P(VCUSUM better) > 97.5% = significant at a=0.05 (one-sided).\n")
    ...
    for m, direction in [("cagr", "higher"), ("mdd", "lower"),
        ...
        if direction == "lower":
            d = vt_boot[m] - vc_boot[m]  # positive = VCUSUM has lower MDD
        else:
            d = vc_boot[m] - vt_boot[m]  # positive = VCUSUM is higher
        ...
        print(f"    D{m:7s}  mean={d.mean():+8.4f}  "
              f"P(VCUSUM {direction:6s})={p_better * 100:5.1f}%  "
              f"95%CI=[{ci[0]:+.4f}, {ci[1]:+.4f}]{sig}")

        paired_results[m] = {
            "mean_delta": round(float(d.mean()), 6),
            "p_vcusum_better": round(p_better, 4),
...
def run_correction(p_cusum, p_atr):
    ...
    # K=14 from VTREND + VBREAK + VPULL
    # +2 for VCUSUM (cusum signal, ATR)
    hyps = [
        ...
        # New VCUSUM hypotheses
        ("VCUSUM cusum signal",        p_cusum),
        ("VCUSUM ATR trail",           p_atr),
    ]
    ...
    # Sort by p-value for Holm and BH
    ...
    cusum_survives = {"bonferroni": False, "holm": False, "bh": False}
    ...
    for rank, (name, p) in enumerate(sorted_hyps):
        ...
        if name == "VCUSUM cusum signal":
            cusum_survives["bonferroni"] = bonf_pass
            cusum_survives["holm"] = holm_pass
            cusum_survives["bh"] = bh_pass

    print(f"\n  VCUSUM cusum signal survives:")
    print(f"    Bonferroni: {'PASS' if cusum_survives['bonferroni'] else 'FAIL'}")
    print(f"    Holm:       {'PASS' if cusum_survives['holm'] else 'FAIL'}")
    print(f"    BH (FDR):   {'PASS' if cusum_survives['bh'] else 'FAIL'}")

    gate4_passed = cusum_survives["bonferroni"]
    print(f"\n  Gate 4: {'PASS' if gate4_passed else 'FAIL'} (Bonferroni)")

    return gate4_passed, cusum_survives, K
...
if __name__ == "__main__":
    print("=" * 70)
    print("VCUSUM — SEQUENTIAL CHANGE-POINT DETECTION: FULL VALIDATION PIPELINE")
    print("=" * 70)
    ...
    print(f"  Timescales: {len(W_GRID)} ({W_GRID[0]}-{W_GRID[-1]} H4 bars)")
    print(f"  CUSUM: k={CUSUM_K}, threshold={THRESH}")
    ...
    output = {
        "config": {
            ...
            "w_grid": W_GRID, "cusum_k": CUSUM_K,
            "threshold": THRESH, "trail": TRAIL,
            "atr_period": ATR_P,
    ...
    if p_cusum >= 0.001:
        ...
        output["fail_reason"] = f"Phase 1: CUSUM signal not significant (p={p_cusum:.6f})"
        outpath = outdir / "vcusum_test.json"
    ...
    p_min, p_max, s_min, s_max, peak_W, prod_width = analyze_timescale(
    ...
    for j, W in enumerate(W_GRID):
        ...
        phase2["real_data"][key] = {
            "with_vdo": {k: round(v, 4) for k, v in real_results[W]["with_vdo"].items()},
            "no_vdo": {k: round(v, 4) for k, v in real_results[W]["no_vdo"].items()},
        }
    ...
    vc_boot, vt_boot = run_paired_bootstrap(cl, hi, lo, vo, tb, wi, peak_W)
    gate3_passed, paired_results = analyze_paired(vc_boot, vt_boot)

    phase3 = {"best_W": peak_W}
    for lab, store in [("vcusum", vc_boot), ("vtrend", vt_boot)]:
        phase3[lab] = {}
        for m in ["cagr", "mdd", "sharpe", "calmar"]:
            a = store[m]
    ...
    if not gate3_passed:
        output["determination"] = "FAIL"
        output["fail_reason"] = "Phase 3: VCUSUM not significantly better than VTREND"
        outpath = outdir / "vcusum_test.json"
        with open(outpath, "w") as f:
            json.dump(output, f, indent=2, default=str)
    ...
    gate4_passed, survives, K = run_correction(p_cusum, p_atr)

    output["phase_4_correction"] = {
        "K": K,
        "alpha": 0.05,
        "bonferroni_threshold": round(0.05 / K, 6),
        "cusum_survives_bonferroni": survives["bonferroni"],
        "cusum_survives_holm": survives["holm"],
        "cusum_survives_bh": survives["bh"],
    }
    ...
    if gate4_passed:
        ...
        output["determination"] = "FAIL"
        output["fail_reason"] = "Phase 4: CUSUM signal does not survive Bonferroni"
