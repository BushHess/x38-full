"""Run price-level distance survey for x35."""
...
def _build_outer_series(d1_df: pd.DataFrame) -> tuple[dict[str, pd.DataFrame], pd.DataFrame, pd.DataFrame]:
    ...
    weekly["wk_dist_to_ema26"] = weekly["close"] / weekly["ema26"] - 1.0
    weekly["wk_dist_to_ema52"] = weekly["close"] / weekly["ema52"] - 1.0

    monthly = aggregate_outer_bars(d1_df, "M1").copy()
    monthly["ema6"] = monthly["close"].ewm(span=6, adjust=False).mean()
    monthly["mo_dist_to_ema6"] = monthly["close"] / monthly["ema6"] - 1.0

    frames = {
        "wk_dist_to_ema26": weekly[["close_time", "wk_dist_to_ema26"]].copy(),
        "wk_dist_to_ema52": weekly[["close_time", "wk_dist_to_ema52"]].copy(),
        "mo_dist_to_ema6": monthly[["close_time", "mo_dist_to_ema6"]].copy(),
    }
    return frames, weekly, monthly
...
def _bucketize(series: pd.Series) -> pd.Series:
    ...
    return pd.cut(series, bins=bins, labels=["Q1", "Q2", "Q3", "Q4"], include_lowest=True, duplicates="drop")
...
def _fold_stability(frame: pd.DataFrame) -> tuple[int, int, list[dict[str, float | int | None]]]:
    ...
    for fold_idx, chunk in enumerate(chunks, start=1):
        ...
        top = fold.loc[fold["bucket"] == "Q4", "return_pct"]
        bottom = fold.loc[fold["bucket"] == "Q1", "return_pct"]
...
def _feature_summary(
    ...
    *,
    ...
    d1_frame["bucket"] = _bucketize(d1_frame[feature])
    ...
    trade_frame["bucket"] = _bucketize(trade_frame[feature])
    ...
    q1 = next(row for row in bucket_rows if row["bucket"] == "Q1")
    q4 = next(row for row in bucket_rows if row["bucket"] == "Q4")
    total_profit = float(trade_frame.loc[trade_frame["pnl"] > 0, "pnl"].sum())
    total_loss = float(-trade_frame.loc[trade_frame["pnl"] < 0, "pnl"].sum())
    profit_share_top = safe_div(q4["profit_sum"], total_profit)
    loss_share_bottom = safe_div(q1["loss_sum"], total_loss)
    ...
    valid_folds, fold_wins, fold_rows = _fold_stability(trade_frame)
    fold_win_rate = safe_div(fold_wins, valid_folds)

    delta_mean = float(q4["mean_return_pct"] - q1["mean_return_pct"])
    delta_win_rate = float(q4["win_rate_pct"] - q1["win_rate_pct"])
    ...
    return {
        ...
        "trade_split": {
            "n_bottom": int(q1["n_trades"]),
            "n_top": int(q4["n_trades"]),
            "top_mean_return_pct": round(q4["mean_return_pct"], 4),
            "bottom_mean_return_pct": round(q1["mean_return_pct"], 4),
            "delta_mean_return_pct_top_minus_bottom": round(delta_mean, 4),
            "top_win_rate_pct": round(q4["win_rate_pct"], 2),
            "bottom_win_rate_pct": round(q1["win_rate_pct"], 2),
...
def _build_markdown(payload: dict) -> str:
    lines = [
        ...
        f"- Warmup months before report: `{payload['warmup']['pre_report_months']}`",
...
def main() -> None:
    ...
    feature_frames, weekly, monthly = _build_outer_series(d1_df)
    pre_report_weeks = int((weekly["close_time"] < report_start_ms).sum())
    pre_report_months = int((monthly["close_time"] < report_start_ms).sum())
    warmup_ok = pre_report_weeks >= 52 and pre_report_months >= 6
    ...
    branch_verdict = "GO_F1_PRICE_LEVEL_FAMILY" if any(row["promising"] for row in feature_rows) else "NO_GO_F1_PRICE_LEVEL_FAMILY"

    payload = {
        "study_id": "x35_long_horizon_regime",
        "branch": "f_price_level_state_diagnostic",
        "branch_verdict": branch_verdict,
        "warmup": {
            "pre_report_weeks": pre_report_weeks,
            "pre_report_months": pre_report_months,
            "warmup_ok": warmup_ok,
        },
        "baseline": {
    ...
    json_path = RESULTS_DIR / "price_level_state_scan.json"
    md_path = RESULTS_DIR / "price_level_state_scan.md"
