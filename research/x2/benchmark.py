#!/usr/bin/env python3
"""X2 Research — Benchmark: adaptive trailing stop vs E0+EMA21(D1) baseline.

Hypothesis: Adapting the trailing stop multiplier based on unrealized gain
improves trend capture.  When a position is deeply in profit (strong trend),
a wider stop avoids whipsaw from normal pullbacks.  When freshly entered
with no buffer, a tighter stop protects capital.

  unrealized_gain < 5%  -> trail_mult = 3.0  (tight)
  unrealized_gain < 15% -> trail_mult = 4.0  (mid)
  unrealized_gain >= 15% -> trail_mult = 5.0 (wide)

Comparison:
  BASELINE = E0+EMA21(D1)  — fixed trail_mult = 3.0
  X2       = E0+EMA21(D1)  — adaptive trail_mult based on unrealized gain
...
Evaluation:
  ...
  T4: Trail tier statistics (how many exits per tier, avg return per tier)
"""
...
STRATEGY_IDS = ["E0_EMA21", "X2"]
STRATEGY_LABELS = {
    "E0_EMA21": "E0+EMA21(D1) [baseline]",
    "X2": "X2 adaptive trail stop",
}
...
def _make_strategy(sid):
    if sid == "E0_EMA21":
        ...
        return VTrendX2Strategy(VTrendX2Config(
            ...
            gain_tier1=GAIN_TIER1,
            gain_tier2=GAIN_TIER2,
        ))
    else:
...
def run_backtests_engine():
    ...
    # Print delta table (X2 - E0_EMA21)
    print(f"\n{'DELTA (X2 - BASELINE)':>30s}")
    print("-" * 80)
    for sc in ["smart", "base", "harsh"]:
    ...
    for sid in STRATEGY_IDS:
        for sc in ["harsh"]:
            m = results[sid][sc]
            print(f"  {sid} ({sc}): {m['exit_reason_counts']}")
...
def _sim_x2(cl, hi, lo, vo, tb, wi, d1_cl, d1_ct, h4_ct,
            ...
            cps=0.005):
    """Vectorized X2 sim with adaptive trailing stop."""
    n = len(cl)
    fast_p = max(5, slow_period // 4)
    ef = _ema(cl, fast_p); es = _ema(cl, slow_period)
    at = _atr(hi, lo, cl, ATR_P)
    ...
    for i in range(n):
        ...
        if i > 0:
            ...
            if pe:
                ...
                inp = True; pk = p; entry_price = fp
            elif px:
                px = False; cash = bq * fp * (1 - cps); bq = 0.0
                inp = False; nt += 1; entry_price = 0.0
        nav[i] = cash + bq * p
        if math.isnan(at[i]) or math.isnan(ef[i]) or math.isnan(es[i]):
            continue
        if not inp:
...
def _run_surrogate(sid, cl, hi, lo, vo, tb, wi, d1_cl, d1_ct, h4_ct, cps=0.005):
    ...
    if sid == "E0_EMA21":
        ...
        return _sim_x2(cl, hi, lo, vo, tb, wi, d1_cl, d1_ct, h4_ct, **kw)
    else:
        raise ValueError(f"Unknown strategy: {sid}")
...
def run_bootstrap(cl, hi, lo, vo, tb, wi, d1_cl, d1_ct, h4_ct):
    ...
    print(f"\n  HEAD-TO-HEAD (X2 - E0_EMA21) across {N_BOOT} bootstrap paths:")
    print(f"    Sharpe: X2 wins {np.sum(h2h_sharpe > 0)}/{N_BOOT} "
          f"({np.mean(h2h_sharpe > 0)*100:.1f}%)  "
          f"mean delta={np.mean(h2h_sharpe):.4f}")
    print(f"    CAGR:   X2 wins {np.sum(h2h_cagr > 0)}/{N_BOOT} "
          f"({np.mean(h2h_cagr > 0)*100:.1f}%)  "
          f"mean delta={np.mean(h2h_cagr):.2f}%")
    print(f"    MDD:    X2 wins {np.sum(h2h_mdd < 0)}/{N_BOOT} "
          f"({np.mean(h2h_mdd < 0)*100:.1f}%)  "
          f"mean delta={np.mean(h2h_mdd):.2f}%")

    results["h2h"] = {
        "sharpe_x2_win_pct": float(np.mean(h2h_sharpe > 0) * 100),
        "sharpe_mean_delta": float(np.mean(h2h_sharpe)),
        "cagr_x2_win_pct": float(np.mean(h2h_cagr > 0) * 100),
        "cagr_mean_delta": float(np.mean(h2h_cagr)),
        "mdd_x2_win_pct": float(np.mean(h2h_mdd < 0) * 100),
...
# =========================================================================
# T4: TRAIL TIER STATISTICS
# =========================================================================

def run_trail_tier_stats():
    """Analyze X2 trades by which trail tier was active at exit."""
    print("\n" + "=" * 80)
    print("T4: ADAPTIVE TRAIL TIER STATISTICS")
    print("=" * 80)
    ...
    for scenario in ["smart", "base", "harsh"]:
        ...
        # Classify each trade by max unrealized gain to infer which tier was active
        tier_counts = {"tight": 0, "mid": 0, "wide": 0}
        tier_pnl = {"tight": 0.0, "mid": 0.0, "wide": 0.0}
        tier_returns = {"tight": [], "mid": [], "wide": []}
        tier_days = {"tight": [], "mid": [], "wide": []}

        trail_exits = [t for t in res.trades if t.exit_reason == "x2_trail_stop"]
        trend_exits = [t for t in res.trades if t.exit_reason == "x2_trend_exit"]

        for t in res.trades:
            # Use return_pct as proxy for unrealized gain at exit
            ret = t.return_pct / 100.0 if hasattr(t, 'return_pct') else 0.0
            if ret < GAIN_TIER1 * 100:  # return_pct is in %, gain thresholds in fraction
                # Actually return_pct is already %, so compare properly
                pass

            # More accurate: use the trade's return to infer tier
            # return_pct is the actual realized return
            ret_frac = t.return_pct / 100.0 if hasattr(t, 'return_pct') else 0.0

            # The max unrealized gain during the trade was >= realized return
            # (since trailing stop locks in less than peak).
            # For trail stop exits: the stop fired when price pulled back from peak.
            # The peak unrealized gain = (peak_price - entry_price) / entry_price
            # We don't have peak_price directly, but we know:
            # - If return < tier1 (5%): was in tight tier at exit
            # - If return >= tier1 but < tier2: could have been in mid or tight
            # - If return >= tier2: was definitely in wide tier at some point

            # Conservative estimate: classify by realized return
            if ret_frac < GAIN_TIER1:
                tier = "tight"
            elif ret_frac < GAIN_TIER2:
                tier = "mid"
            else:
                tier = "wide"
            ...
            tier_counts[tier] += 1
            tier_pnl[tier] += t.pnl
            tier_returns[tier].append(t.return_pct)
            tier_days[tier].append(t.days_held)

        n_total = len(res.trades)
        print(f"\n  {scenario} scenario ({n_total} total trades, "
              f"{len(trail_exits)} trail stops, {len(trend_exits)} trend exits):")
        print(f"    {'Tier':8s} {'Count':>6s} {'Pct':>6s} {'AvgRet%':>8s} {'AvgDays':>8s} {'TotalPnL':>10s} {'WR%':>6s}")
        print(f"    {'-'*54}")
        for tier in ["tight", "mid", "wide"]:
            cnt = tier_counts[tier]
            pct = cnt / n_total * 100 if n_total > 0 else 0
            avg_ret = np.mean(tier_returns[tier]) if tier_returns[tier] else 0
            avg_days = np.mean(tier_days[tier]) if tier_days[tier] else 0
            total_pnl = tier_pnl[tier]
            wr = (sum(1 for r in tier_returns[tier] if r > 0) / cnt * 100) if cnt > 0 else 0
            print(f"    {tier:8s} {cnt:6d} {pct:5.1f}% {avg_ret:8.2f} {avg_days:8.1f} {total_pnl:10.2f} {wr:5.1f}%")

    # Also run baseline for comparison
    print(f"\n  BASELINE (E0+EMA21) harsh scenario:")
...
def save_results(bt_results, boot_results, parity_data):
    ...
    json_path = OUTDIR / "x2_results.json"
    with open(json_path, "w") as f:
        json.dump(out, f, indent=2, default=str)
    print(f"\nSaved: {json_path}")

    # CSV backtest table
    csv_path = OUTDIR / "x2_backtest_table.csv"
    ...
    # CSV bootstrap table
    csv_boot = OUTDIR / "x2_bootstrap_table.csv"
    ...
    # CSV delta table
    csv_delta = OUTDIR / "x2_delta_table.csv"
...
def main():
    print("=" * 80)
    print("X2 RESEARCH: Adaptive Trailing Stop Benchmark")
    print(f"  Data: {DATA}")
    print(f"  Period: {START} to {END} (warmup={WARMUP}d)")
    print(f"  Params: slow={SLOW}, trail_tight={TRAIL_TIGHT}, trail_mid={TRAIL_MID}, "
          f"trail_wide={TRAIL_WIDE}")
    print(f"  Gain tiers: <{GAIN_TIER1*100:.0f}%=tight, <{GAIN_TIER2*100:.0f}%=mid, "
          f">={GAIN_TIER2*100:.0f}%=wide")
